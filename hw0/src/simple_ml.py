...
def parse_label(filename):
    ...
    _, number_of_items = struct.unpack(f"{byteorder()}2l", bytes[:8])

    labels = []
    for (label,) in struct.iter_unpack(f"{byteorder()}B", bytes[8:]):
        labels.append(label)
...
def softmax_loss(Z, y):
    ...
    """
    return np.average(np.log(np.sum(np.exp(Z), axis=1)) - Z[np.indices(y.shape)[0], y])
...
def exp_normalized(data, theta):
    temp = np.exp(data @ theta)
    return temp / (temp @ np.ones((theta.shape[1], 1)))
...
def softmax_regression_epoch(X, y, theta, lr=0.1, batch=100):
    ...
    one_hots = np.eye(theta.shape[1])
    for ind in range(batch_num):
        data = X[ind * batch : (ind + 1) * batch, :]
        label = y[ind * batch : (ind + 1) * batch]
        Z = exp_normalized(data, theta)
        gradient = (data.T @ (Z - one_hots[label])) / data.shape[0]
...
def nn_epoch(X, y, W1, W2, lr=0.1, batch=100):
    ...
    for ind in range(batch_num):
        ...
        label = y[ind * batch : (ind + 1) * batch]
        Z1 = relu(data @ W1)
        G2 = exp_normalized(Z1, W2) - one_hot(label, W2.shape[1])
        G1 = np.where(Z1 > 0, 1, 0) * (G2 @ W2.T)

        gradient_w1 = data.T @ G1 / data.shape[0]
        gradient_w2 = Z1.T @ G2 / data.shape[0]
