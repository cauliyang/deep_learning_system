...
def test_divide_forward():
    np.testing.assert_allclose(
        ...
        np.array([[0.717391304348, 1.0, 0.25], [3.769230769231, 1.357142857143, 0.579545454545]]),
    )
...
def test_matmul_forward():
    np.testing.assert_allclose(
        ...
        np.array([[13.7075, 19.5025, 13.26], [22.31, 29.785, 18.7275], [4.36, 4.365, 10.22]]),
    )
    np.testing.assert_allclose(
        ndl.matmul(
        ...
        np.array([[4.1825, 13.3625, 14.11], [2.6975, 12.2375, 11.86], [1.89, 8.85, 8.52]]),
    )
    np.testing.assert_allclose(
        ndl.matmul(
            ndl.Tensor(
        ...
        np.array(
            [
                [[14.5625, 22.7575, 5.345], [5.7375, 9.18, 3.23], [4.825, 7.9175, 3.755]],
    ...
    np.testing.assert_allclose(
        ...
        np.array(
            [
                [[9.5, 8.835, 7.98], [24.375, 22.605, 20.385], [18.125, 16.5375, 14.775]],
    ...
    np.testing.assert_allclose(
        ...
        np.array(
            [
                [[14.855, 6.045, 8.9425], [5.425, 2.6975, 0.9175], [20.68, 8.58, 11.66]],
...
def test_summation_forward():
    ...
    np.testing.assert_allclose(
        ndl.summation(
            ndl.Tensor([[1.5, 3.85, 3.45], [1.35, 1.3, 0.65], [2.6, 4.55, 0.25]]), axes=0
        ).numpy(),
...
def test_reshape_forward():
    np.testing.assert_allclose(
        ...
        np.array(
            [2.9, 2.0, 2.4, 3.95, 3.95, 4.65, 2.1, 2.5, 2.7, 1.9, 4.85, 3.25, 3.35, 3.45, 3.45]
        ),
    )
    np.testing.assert_allclose(
        ndl.reshape(
        ...
        np.array(
            [
                [[4.1, 4.05, 1.35, 1.65], [3.65, 0.9, 0.65, 4.15], [4.7, 1.4, 2.55, 4.8]],
...
def test_transpose_forward():
    ...
    np.testing.assert_allclose(
        ndl.transpose(ndl.Tensor([[[[0.95]]], [[[2.55]]], [[[0.45]]]]), axes=(2, 3)).numpy(),
...
def submit_forward():
    ...
    mugrade.submit(ndl.transpose(ndl.Tensor([[[3.45]], [[2.54]], [[1.91]]]), axes=(0, 1)).numpy())
    mugrade.submit(ndl.transpose(ndl.Tensor([[4.45, 2.15], [1.89, 1.21], [6.15, 2.42]])).numpy())
...
def gradient_check(f, *args, tol=1e-6, backward=False, **kwargs):
    ...
    if not backward:
        out = f(*args, **kwargs)
        computed_grads = [
            x.numpy() for x in out.op.gradient_as_tuple(ndl.Tensor(np.ones(out.shape)), out)
        ]
    else:
...
def test_divide_backward():
    gradient_check(
        ndl.divide, ndl.Tensor(np.random.randn(5, 4)), ndl.Tensor(5 + np.random.randn(5, 4))
    )
...
def test_matmul_batched_backward():
    ...
    gradient_check(
        ndl.matmul, ndl.Tensor(np.random.randn(6, 6, 5, 4)), ndl.Tensor(np.random.randn(4, 3))
    )
    gradient_check(
...
def submit_backward():
    np.random.seed(0)
    out = gradient_check(
        ndl.divide, ndl.Tensor(np.random.randn(3, 5)), ndl.Tensor(6 + np.random.randn(3, 5))
    )
    print(out)
    mugrade.submit(out)
    mugrade.submit(
...
def test_compute_gradient():
    ...
    gradient_check(
        lambda A, B, C: ndl.summation(ndl.reshape(A, shape=(10, 10)) @ B / 5 + C, axes=None),
...
def test_softmax_loss_ndl():
    ...
    np.testing.assert_allclose(
        ndl.log(ndl.Tensor([[4.0], [4.55]])).numpy(), np.array([[1.38629436112], [1.515127232963]])
    )
...
def submit_softmax_loss_ndl():
    ...
    y = ndl.Tensor(y_one_hot)
    mugrade.submit(
        softmax_loss(ndl.Tensor(np.zeros((y.shape[0], 10)).astype(np.float32)), y).numpy()
    )
    np.random.seed(0)
    mugrade.submit(
        softmax_loss(ndl.Tensor(np.random.randn(y.shape[0], 10).astype(np.float32)), y).numpy()
    )
...
def test_nn_epoch_ndl():
    ...
    dW1 = nd.Gradient(
        lambda W1_: softmax_loss(ndl.relu(X_ @ ndl.Tensor(W1_).reshape((5, 10))) @ W2, y_).numpy()
    )(W1.numpy())
    dW2 = nd.Gradient(
        lambda W2_: softmax_loss(ndl.relu(X_ @ W1) @ ndl.Tensor(W2_).reshape((10, 3)), y_).numpy()
    )(W2.numpy())
    ...
    np.testing.assert_allclose(np.linalg.norm(W1.numpy()), 28.437788, rtol=1e-5, atol=1e-5)
    np.testing.assert_allclose(np.linalg.norm(W2.numpy()), 10.455095, rtol=1e-5, atol=1e-5)
    np.testing.assert_allclose(
